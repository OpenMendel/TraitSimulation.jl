...
# function Generate_Random_Model_Chisq(filepath::String, k::Int64, n::Int64; column_index)
# snpdata = SnpArray(filepath * ".bed")
# rowmask, colmask =  SnpArrays.filter(snpdata)
# if !isempty(column_index)
# 	copyto!(colmask, column_index)
# end

# x = SnpArrays.filter(filepath, rowmask, colmask)
# SNP_data = SnpData(filepath)

# # SnpData(SnpArrays.datadir(filepath *".filtered"))
